...
if (global.state != GameState.ROUND_ACTIVE) {
    ...
    miniboss_spawned = false;  // reset for next round
...
if (!spawn_initialized) {
    ...
    // Reset miniboss_spawned at the start of round 5
...
// --- Special round 5: spawn exactly one miniboss, no other enemies ---
if (global.round_num == 5) {
    if (!miniboss_spawned) {
        // Spawn miniboss somewhere safe from player
        var tries = 0;
        var max_tries = 20;
        var spawn_ok = false;
        var _x, _y;
        ...
        if (spawn_ok) {
            instance_create_layer(_x, _y, "Instances", OBJ_MiniBoss);
