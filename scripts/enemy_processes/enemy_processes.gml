...
function enemy_anim() {
    if (state == states.DEAD) {
        ...
        if (sprite_index != s_dead_selected) {
            ...
            // Blood burst effect - FIXED: Check if Gorilla exists first
            var _dir = 0; // Default direction
            if (instance_exists(OBJ_Gorilla)) {
                _dir = point_direction(x, y, OBJ_Gorilla.x, OBJ_Gorilla.y) + 180;
            } else {
                // Use a random direction if Gorilla doesn't exist
                _dir = random(360);
            }
